...
int get_dp(int x,int y,int l,int r,int u,int d,int k){
    ...
    if(!k){
        ...
        int t2=0;
        for(int i=1;i<=4;i++){
            if(i==u || i==l || i==r) continue;
            for(int j=1;j<=4;j++){
                if(j==d || j==l || j==r || j==i) continue;
                t2++;
            }
        }
        dp[x][y][l][r][u][d][k]=(dp[x][y][l][r][u][d][k]+MOD-t2*(x-1))%MOD;
        for(int i=1;i<5;i++)
            if(i!=l&&i!=r&&i!=u&&i!=d)
                dp[x][y][l][r][u][d][k]++;
    }
        ...
        int t2=0;
        for(int i=1;i<=4;i++){
            if(i==l || i==u || i==d) continue;
            for(int j=1;j<=4;j++){
                if(j==r || j==u || j==d || j==i) continue;
                t2++;
            }
        }
        dp[x][y][l][r][u][d][k]=(dp[x][y][l][r][u][d][k]+MOD-t2*(y-1))%MOD;
        for(int i=1;i<5;i++)
            if(i!=l&&i!=r&&i!=u&&i!=d)
                dp[x][y][l][r][u][d][k]++;
    }
