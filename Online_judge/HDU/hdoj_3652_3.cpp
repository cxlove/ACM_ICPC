...
//k为0表示没有出现133//k为1表示没有出现13，但是首位为3
//k为2表示出现13
int bit[15],len;
//分别表示当前考虑的位置，前一个数字，当前余数，是否有限制，是否已经出现13
int dfs(int pos,int pre,int mod,bool limit,bool flag){
    if(pos<=0) return flag&&(mod==0);
    if(!limit&&flag&&~dp[pos][mod][2])  return  dp[pos][mod][2];
    if(!limit&&!flag&&pre!=1&&~dp[pos][mod][1]) return dp[pos][mod][1];
    if(!limit&&!flag&&pre==1&&~dp[pos][mod][0]) return dp[pos][mod][0];
    int end=(limit?bit[pos]:9);
    int ret=0;
    for(int i=0;i<=end;i++)
        ret+=dfs(pos-1,i,(mod*10+i)%13,limit&&(i==end),flag||(pre==1&&i==3));
    if(!limit){
        if(pre==1&&!flag) dp[pos][mod][0]=ret;
        if(pre!=1&&!flag) dp[pos][mod][1]=ret;
        if(flag) dp[pos][mod][2]=ret;
    }
    return ret;
}
int slove(int n){
