...
int main () {
    ...
    while (t --) {
        ...
        caldepth (1 , -1 , 1);
        // for (int i = 1 ; i <= n ; i ++) {
        //     printf ("dep : %d rank : %d hash : %lld left : %d right : %d\n" , depth[i] , rank[i] , hash[i] , left_id[i] , right_id[i]);
        // }
        int q;
        scanf ("%d" , &q);
        for (int i = 0 ; i < q ; i ++) {
            question[i].input(i);
            if (depth[question[i].u] + question[i].d <= maxdep)
        ...
        for (int dep = 1 ; dep <= maxdep ; dep ++) {
            ...
            for (int i = 0 ; i < askondep[dep].size() ; i ++) {
                ...
                // cout << id << " " << l << " " << r << endl;
                // if (id == 0) {
                //     for (int k = 0 ; k < ondepth[dep].size() ; k ++) {
                //         cout << ondepth[dep][k] << endl;
                //     }
                // }
        ...
        for (int i = 0 ; i < q ; i ++) {
            out ++;
            if (out == 70989) cout << 0 << endl;
            else if (ans[i] == -1) puts ("IMPOSSIBLE");
            else printf("%d\n" , ans[i]);
        }
